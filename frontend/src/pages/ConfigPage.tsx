        ...
        <div style={{ padding: '20px' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '300px' }}>
                <div style={inputStyle}>
                    ...
                    <input placeholder="e.g. M001" value={formData.machine_id} onChange={e => setFormData({ ...formData, machine_id: e.target.value })} />
                </div>
                <div style={inputStyle}>
                    <label style={labelStyle}>Material</label>
                    <input placeholder="e.g. AL-6061" value={formData.material} onChange={e => setFormData({ ...formData, material: e.target.value })} />
                </div>
                <div style={inputStyle}>
                    <label style={labelStyle}>Tool Diameter (mm)</label>
                    <input type="number" placeholder="10.0" value={formData.tool_diameter} onChange={e => setFormData({ ...formData, tool_diameter: parseFloat(e.target.value) })} />
                </div>
                <div style={inputStyle}>
                    <label style={labelStyle}>Feed Rate (mm/min)</label>
                    <input type="number" placeholder="1000" value={formData.feed_rate} onChange={e => setFormData({ ...formData, feed_rate: parseFloat(e.target.value) })} />
                </div>
                <div style={inputStyle}>
                    <label style={labelStyle}>Spindle Speed (RPM)</label>
                    <input type="number" placeholder="10000" value={formData.spindle_speed} onChange={e => setFormData({ ...formData, spindle_speed: parseInt(e.target.value) })} />
                </div>
                <button type="submit" style={{ marginTop: '10px' }}>Create Configuration</button>
            </form>
